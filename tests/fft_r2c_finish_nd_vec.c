...
#define DIMS (7)
static const int size[DIMS] = {4, 5, 3, 7, 8, 6, 11};
...
int main(int argc, char *argv[])
{
  ...
  fftw_plan fwd_plan = fftw_plan_many_dft_r2c(DIMS, size, VL,
  ...
  fft_r2c_finish_packed_vec(fft, DIMS, size, VL);

  // Allocate the reverse FFT destination array
  double complex *dst = fftw_malloc(netsize*VL*sizeof(double complex));
  if(dst == NULL) {
  ...
  fftw_plan rev_plan = fftw_plan_many_dft(DIMS, size, VL,
      fft, NULL, VL, 1, dst, NULL, VL, 1, FFTW_BACKWARD, FFTW_ESTIMATE);
  if(rev_plan == NULL) {
  ...
  double norm = 0.0;
  for(unsigned int i = 0; i < netsize*VL; ++i) {
