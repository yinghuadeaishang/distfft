...
int main(int argc, char *argv[])
{
  ...
  double *src = fftw_malloc(size[0]*size[1]*VL*sizeof(double));
  if(src == NULL) {
    fprintf(stdout, "unable to allocate source vector\n");
    goto skip_deallocate_src;
  }
  for(unsigned int i = 0; i < size[0]*size[1]*VL; ++i) {
    src[i] = 2*dsfmt_genrand_open_close(dsfmt) - 1;
  }

  // Allocate the FFT destination array
  double complex *fft = fftw_malloc(size[1]*size[0]*VL*sizeof(double complex));
  if(fft == NULL) {
  ...
  fftw_plan fwd_plan = fftw_plan_many_dft_r2c(2, size, VL, src, NULL, VL, 1,
      fft, NULL, VL, 1, FFTW_ESTIMATE);
  if(fwd_plan == NULL) {
  ...
  fft_r2c_finish_packed_vec(fft, 2, size, VL);

  // Allocate the reverse FFT destination array
  double complex *dst = fftw_malloc(size[0]*size[1]*VL*sizeof(double complex));
  if(dst == NULL) {
  ...
  fftw_plan rev_plan = fftw_plan_many_dft(2, size, VL, fft, NULL, VL, 1, dst,
      NULL, VL, 1, FFTW_BACKWARD, FFTW_ESTIMATE);
  if(rev_plan == NULL) {
  ...
  double norm = 0.0;
  for(int i = 0; i < size[0]*size[1]*VL; ++i) {
